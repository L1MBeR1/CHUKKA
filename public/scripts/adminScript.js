...
function populateTable(data, currentTableName) {
  ...
  tbody.innerHTML = '';
  const hiddenColumns = ['id_user','id_discounted_shoes','id_shoe','id_sale','id_size','id_brand','id_review','id_bucket_row'];
  // const hiddenColumns =[''];
...
function removefunc(data, category) {
  ...
  fetch(`/admin/removeData/${category}/${id}`, {
    ...
    .then(response => {
      if (response.ok) {
        ...
        response.json().then(errorData => {
          ...
          putmessage(`Ошибка удаления данных. Код состояния: ${response.status}, Текст ошибки: ${errorData.message}`);
        }).catch(parseError => {
          console.error('Ошибка при парсинге JSON:', parseError);
          putmessage(`Ошибка удаления данных. Код состояния: ${response.status}, Текст ошибки: ${response.statusText}`);
        });
    ...
    .catch(error => {
      console.error('Ошибка загрузки данных:', error);
      if (error.message) {
        putmessage(`Ошибка выполнения операции: ${error.message}`);
      } else {
        putmessage('Произошла ошибка при выполнении операции');
      }
    });
}
function add(){
...
function deleteButtons(category){
var container = document.querySelector('.buttonContainer')
if ((category =='Строки корзины'||category =='Заказы')&&container){
  console.log(1231)
...
function chooseOrder(rowData,row){
  if (rowData.payment_status=='Не оформлен'){
    alert('Данные заказ ещё не оформлен');
    return;
  }
  console.log(rowData,row)

  var window = document.getElementsByClassName('chooseOrderWindow')[0];
  var content = document.getElementsByClassName('chooseOrderWindow-content')[0];
  var CollectBtn = document.getElementsByClassName('bucket_rowOrderCollect')[0];
  var closeBtn = document.getElementsByClassName('bucket_rowOrderClose')[0];
  var table = document.getElementById("OrderTable");
  content.querySelector('h1').innerHTML = 'Товары заказа';
...
fetch(`/admin/getBucket_rowForOrder/${rowData.id_order}`)
    .then(response => response.json())
    .then(data => {
        console.log(data);

        // Проверка, есть ли данные для создания заголовков
        if (data.length > 0) {
            var thead = table.createTHead();
            var headerRow = thead.insertRow(0);

            // Задание собственных названий колонкам
            var columnNames = ['Название модели', 'Бренд', 'Количество пар', 'Размер'/* и так далее */];
