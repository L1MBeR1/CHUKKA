...
router.get('/getBucket_rowForOrder/:id', async (req, res) => {
  try {
      const { id } = req.params;
      const data = await bd.query('SELECT * FROM public.get_bucket_rows_by_order_id($1);',[id]);
      console.log(data);
      res.json(data);
  } catch (error) {
      console.error('Ошибка загрузки данных:', error);
...
router.put('/UpdateOrder', async (req, res) => {
  try {
    ...
    // Ваш код для вставки данных в базу данных
    // Например, используя ваш модуль работы с базой данных (bd.js)
    const result = await bd.query('SELECT * FROM public.update_order_status_to_shipped($1);',[id]);
    const { message, success } = result[0];
    console.log(result[0]);
    if (success) {
      res.status(201).json({ message: 'Заказ успешно отправлен', success: true });
    } else {
      res.status(400).json({ message, success: false });
    }
  } catch (error) {
